...
local siriusValues = {
	...
	pingProfile = {
		...
		adaptiveBaselineSamples = 30, -- how many samples Aureon takes before deciding on a fixed high ping value
...
local aureonSettings = {
		...
		categorySettings = {
			{
				...
				description = 'Randomise your username in real-time in any CoreGui parented interface, including Aureon. You will still appear as your actual name to others in-game. This setting can be performance intensive.',
				settingType = 'Boolean',
				current = false,
			...
			{
				name = 'Chat Spy',
				description = 'This will only work on the legacy Roblox chat system. Aureon will display whispers usually hidden from you in the chat box.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'When active, Aureon will notify you when the next song in your Music queue plays.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'Cancel any kick request involving you sent by the client. This may prompt detection from Adonis or similar anti-cheats. You will need to rejoin and re-run Aureon to toggle.',
				settingType = 'Boolean',
				current = false,
	...
	{
		name = 'Keybinds',
		description = 'Assign keybinds to actions or change keybinds such as the one to open/close Aureon.',
	...
	{
		name = 'Performance',
		description = 'Tweak and test your performance settings for Roblox in Aureon.',
		color = Color3.new(1, 0.376471, 0.168627),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Artificial FPS Limit',
				description = 'Aureon will automatically set your FPS to this number when you are tabbed-in to Roblox.',
				settingType = 'Number',
				values = {20, 5000},
				current = 240,
			...
			},
			{
				...
				description = 'Aureon will automatically set your FPS to 60 when you tab-out or unfocus from Roblox.',
			...
			},
			{
				...
				description = 'Aureon will check your average latency in the background and notify you if your current latency significantly goes above your average latency.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'Aureon will check your average FPS in the background and notify you if your current FPS goes below a specific number.',
				settingType = 'Boolean',
				current = true,
	...
	{
		name = 'Detections',
		description = 'Aureon detects and prevents anything malicious or possibly harmful to your wellbeing.',
		...
		categorySettings = {
			...
			},
			{
				...
				description = 'Be notified whenever Aureon detects a player joins your session that could be a game moderator.',
				settingType = 'Boolean', 
				minimumLicense = 'Pro',
				current = true,
...
-- Initialise Aureon Client Interface
local guiParent = gethui and gethui() or coreGui
local sirius = guiParent:FindFirstChild("Aureon")
if sirius then
	sirius:Destroy()
end
...
local function createEsp(player)
	if player == localPlayer or not checkAureon() then 
		return
	end
	...
	player.CharacterAdded:Connect(function(character)
		if not checkAureon() then return end
		task.wait()
...
local function blurSignature(value)
	if not value then
		if lighting:FindFirstChild("AureonBlur") then
			lighting:FindFirstChild("AureonBlur"):Destroy()
		end
	else
		if not lighting:FindFirstChild("AureonBlur") then
			local blurLight = Instance.new("DepthOfFieldEffect", lighting)
			blurLight.Name = "AureonBlur"
...
local function checkLastVersion()
	...
	if lastVersion then
		if lastVersion ~= siriusValues.siriusVersion then queueNotification("Aureon has been updated", "Aureon has been updated to version "..siriusValues.siriusVersion..", check our Discord for all new features and changes.", 4400701828)  end
	end
...
local function removeReverbs(timing)
	...
	for index, sound in next, soundInstances do
		if sound:FindFirstChild("AureonAudioProfile") then
			local reverb = sound:FindFirstChild("AureonAudioProfile")
...
local function addToQueue(file)
	...
	checkFolder()
	if not isfile(siriusValues.siriusFolder.."/Music/"..file) then queueNotification("Unable to locate file", "Please ensure that your audio file is in the Aureon/Music folder and that you are including the file extension (e.g mp3 or ogg).", 4370341699) return end
...
local function createReverb(timing)
	for index, sound in next, soundInstances do
		if not sound:FindFirstChild("AureonAudioProfile") then
			local reverb = Instance.new("EqualizerSoundEffect")

			reverb.Name = "AureonAudioProfile"
			reverb.Parent = sound
	...
	for _, gameFound in pairs(siriusValues.games) do
		if gameFound.id == placeId and gameFound.enabled then
			...
			gameDetectionPrompt.Layer.Run.MouseButton1Click:Connect(function()
				closeGameDetection()
				queueNotification("Running "..gameFound.name, "Now running Aureon' "..gameFound.name.." script, this may take a moment.", 4400701828)
				runScript(rawFile)
...
local function sortActions()	
	...
	for _, slider in ipairs(siriusValues.sliders) do
		...
		newSlider.Interact.MouseButton1Down:Connect(function()
			if debounce or not checkAureon() then return end
...
if Essential or Pro then
	getgenv()[index] = function(data)
		if checkAureon() and checkSetting("Intelligent HTTP Interception").current then
			local title = "Do you trust this source?"
			local content = "Aureon has prevented data from being sent off-client, would you like to allow data to be sent or retrieved from this source?"
			...
			if url == "http://127.0.0.1:6463/rpc?v=1" then
				...
				if bodyDecoded.cmd == "INVITE_BROWSER" then
					...
					content = "Aureon has prevented your Discord client from automatically joining this Discord server, would you like to continue and join, or block it?"
	...
	getgenv()[indexSetClipboard] = function(data)
		if checkAureon() and checkSetting("Intelligent Clipboard Interception").current then
			local title = "Would you like to copy this to your clipboard?"
			local content = "Aureon has prevented a script from setting the below text to your clipboard, would you like to allow this, or prevent it from copying?"
...
local function onChatted(player, message)
	...
	if not message or not checkAureon() then return end
	...
	if enabled and player ~= localPlayer then
		...
		if hidden and enabled then
			chatSpyVisuals.Text = "Aureon Spy - [".. player.Name .."]: "..message2
	...
	if checkSetting("Client-Based Anti Kick").current then
		if hookmetamethod then 
			...
			originalIndex = hookmetamethod(game, "__index", function(self, method)
				if self == localPlayer and method:lower() == "kick" and checkSetting("Client-Based Anti Kick").current and checkAureon() then
					queueNotification("Kick Prevented", "Aureon has prevented you from being kicked by the client.", 4400699701)
			...
			originalNamecall = hookmetamethod(game, "__namecall", function(self, ...)
				if self == localPlayer and getnamecallmethod():lower() == "kick" and checkSetting("Client-Based Anti Kick").current and checkAureon() then
					queueNotification("Kick Prevented", "Aureon has prevented you from being kicked by the client.", 4400699701)
					return
				end
...
userInputService.InputEnded:Connect(function(input, processed)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		for _, slider in pairs(siriusValues.sliders) do
			...
			if characterPanel.Visible and not debounce and slider.object and checkAureon() then
